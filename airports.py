...
from env import AIRPORTS_API, ACCESS_KEYS, PROXY


def request(access_key, offset):
    ...
    response = requests.get(AIRPORTS_API, params, proxies=PROXY)
...
def flatten(body_data):
    ...
    for elem in body_data:
        ...
        flight = (
            timestamp,
            elem.get("airport_name"),
            elem.get("icao_code"),
            elem.get("latitude"),
            elem.get("longitude"),
            elem.get("country_name"),
            elem.get("country_iso2"),
        )
...
def main():
    ...
    while True:
        ...
        body, url = request(key, offset=c * 100)
